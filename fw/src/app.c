...
/* FRAM 로드+LCD 부팅.
 * [rst 배너] 리셋 원인 — CSR 플래그는 POR 또는 RMVF 로만 지워지므로 읽은 뒤 즉시 클리어
 ...
 * [LCD ready] Cold-boot race fix: the DGUS panel boots slower than the MCU, so the old
 ...
 * [logo dwell] Show the logo splash for a deliberate dwell, then switch to the run page
 ...
 * [boot_complete] Boot handshake done: now honor panel SYS_PIC_NOW re-init reports (spec §10).
