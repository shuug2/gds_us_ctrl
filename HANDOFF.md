...
**Releases**: `releases/README.md` = 공통 SWD 플래시 가이드(CubeProgrammer CLI / OpenOCD / J-Link · SHA 대조 · LCD 버전 판정 · 실패 표 · 보관 절차 명령 + 버전 README 템플릿), 원격기 `gds_us_remote/releases/` 와 같은 목차(2026-09-13 원격기 세션 요청). **첫 보관본 `releases/V3.1.0R_260911/` 생성(2026-09-13, 사용자 지시로 벤치 전)** — REMOTE `.bin` 만(STD 미검증 제외, `.elf` 제외), 클린 빌드 해시 `2a813206…` = 보드 플래시 이미지와 동일 → 재플래시 불요. **태그 미발행·벤치 미통과**를 버전 README 에 명기. 벤치가 코드를 바꾸면 새 날짜 폴더로 대체, PASS 후 태그 발행 시 README 표의 태그 칸만 채운다(원격기에 그때 통보 — 그쪽 CLAUDE.md §7 서술 갱신 대기). 양산 플래시는 반드시 보관본으로, `fw/build*/` 금지
