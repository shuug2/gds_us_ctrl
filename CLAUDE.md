...
**현재 진행 (2026-07-08)**: **사용자 신규 3건 전부 코드-완료**(전건 cpp-review 0 Crit/High, main 직접 커밋): ① **부팅 딜레이 터치 유령 런 fix**(`e26e15b` — V30 RUN data=0 런-상태 매핑이 silent-reject마다 페어링 반전 → 물리 토글 `s_run_key_down`+SYS_PIC_NOW 재앵커로 교체; ⚠1차 reg-레이어 안은 force-stop 경로 재반전으로 폐기=HANDOFF Failed Approaches) ② **REMOTE icon**(`60792da` — samd20 case 9 포팅, 유효 Modbus 요청 동안 ON/1s 후 OFF; V30 에셋 0x120e 렌더는 벤치 첫 확인) ③ **전류 표시 EMA α 1/8→1/2**(`78a1e43`, τ≈100ms; 리뷰 MEDIUM=energy-exit/OVTIME 타이밍 벤치 재확인). **⚠ 보드 미플래시**(구 `434e007` 코드 잔류) + **push 미실행**(main 4±커밋 + 이월 태그 `hw-revA_fw-stage-mbtcp-hardening`, 사람 터미널). 남은 것 = **① FW 벤치 검증**(체크리스트=루트 `HANDOFF.md` §Resume: 전류 0.60A 이월 포함) + **② HMI Task 8**(별도 repo, RS-485) + **③ 6b·B-SEAM**(사용자 보류). 상세 진입 = 루트 `HANDOFF.md`(2026-07-08판) + `docs/NEXT_STEPS.md`, 세션별 상태 = `docs/superpowers/RESUME.md`(자동 로드), 변경 이력 = `docs/changelog.md`.
