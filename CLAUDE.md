...
**현재 진행 (2026-07-06)**: **modbus-tcp-hardening(M6/M8/M9) HW E2E 전항목 PASS + MERGED**(tag `hw-revA_fw-stage-mbtcp-hardening`) — **2026-07-02 감사 큐 완전 소진**. 부수: vendored socket.c recv() 논블로킹 순서-뒤집힘 우회(ctlsocket 토글) + W5500 keep-alive 10s(케이블 분리 stale-ESTABLISHED 락아웃=선재 결함 자가치유) + RS-485 첫-write 조사 보고(research/). 같은 세션: **seek/reset 중 측정값 라이브 표시**(us_on_status 복원, 벤치 PASS) + **표시 전류 전달함수 재정의**(재앵커+오프셋 제거: GAIN 59/126·OFFSET 0·DEADBAND 20 — ⚠0.60A 표시 최종 벤치 확인 미기록) + **overload 실동작 E2E 사용자 PASS**(physical-io 이월 전부 소진). **⚠ push 미실행**(main 18±커밋+태그, 사람 터미널). 남은 것 = **⓪ 사용자 신규 3건**(부팅 딜레이 터치 에러 / REMOTE icon 점등 / 전류 필터 반응 ×2 — 상세는 작업 시작 시 사용자 설명) + **HMI Task 8**(별도 repo, RS-485) + **6b·B-SEAM**(사용자 보류). 상세 진입 = 루트 `HANDOFF.md`(2026-07-06판) + `docs/NEXT_STEPS.md`, 세션별 상태 = `docs/superpowers/RESUME.md`(자동 로드), 변경 이력 = `docs/changelog.md`.
