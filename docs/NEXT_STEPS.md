...
>
> **★ 2026-09-06 — 원격 hold-to-run 워치독 코드-완료, HW 벤치 대기.** 브랜치 `feat/hold-to-run-wdt`(main +8). 새 레지스터 `0x32 FEAT_CAP`(bit0), `START(0x1B)` 값 1=탭/2=hold/3=유지, T=600 ms, host 17스위트. 정본 = `docs/superpowers/specs/2026-09-06-remote-hold-to-run-design.md`, 벤치 = 그 §8 H-0~H-16(**H-0 은 새 빌드 플래시 전 현 보드로**, 플래시 직전 `define.h` 날짜 갱신 필수), 도구 `docs/superpowers/tools/mb_hold.py`. 계약 확정 통보(원격기·HMI)는 벤치 PASS 후. PC8 과 독립.
>
