...
>
> **★ 2026-09-06 — 원격 hold-to-run 워치독 HW 벤치 PASS·main 머지 완료**(`9b8e53b` `--no-ff`, 태그 `hw-revA_fw-stage-hold-wdt`, 브랜치 삭제). 새 레지스터 `0x32 FEAT_CAP`(bit0), `START(0x1B)` 값 1=탭/2=hold/3=유지, T=600 ms(실측 603~608), `MB_REG_COUNT` 51(여유 6), host 17스위트. 정본 = `docs/superpowers/specs/2026-09-06-remote-hold-to-run-design.md`, 벤치 = `plans/2026-09-06-hold-to-run-bench-results.md`(실행 16항목 PASS, 소크 6000 keep 이상 0). **부수 수정 `96dc7d5`: horn 모드 중 진행 런 강제 정지**(벤치가 찾은 기존 결함). 원격기 계약 확정 통보·실기 통과 완료. 같은 날 **릴리즈 3.1.0 컷**(`hw-revA_fw-3.1.0`; 번호=티어·날짜=빌드, `define.h` `REMOTE_EN_INTERLOCK_INVERTED` 가 극성+`!` 표식). ⬜ 남은 것: `gds_us_hmi` 통보 · PC8 실장 PCB. **벤치 함정 추가** = LCD 조작이 벤치를 오염(SETUP 저장이 horn down 재전송) / 침묵 >10 s 뒤 첫 프레임 게이트 거부 / warm-up 중 START 무음 거부 — `bench-results` §4.
>
