...
| Pin | Net Name | Signal | 설명 |
|-----|----------|--------|------|
| PB0 | ADC_SENS_OUT | ADC1_IN8 | 센서 출력 아날로그 |
| PB1 | ADC_OSC | ADC1_IN9 | 발진기 모니터 아날로그 |
...
|-----|----------|------------------|------|
| PB2  | CTRL_OSC0 | **mega16 PB1** | 초음파 발진회로 채널 0 ON/OFF |
| PB10 | CTRL_OSC1 | **mega16 PB0** | 초음파 발진회로 채널 1 ON/OFF |
| PB12 | CTRL_OSC2 | **mega16 PC4** | 초음파 발진회로 채널 2 ON/OFF |
| PB13 | CTRL_OSC3 | **mega16 PA7** | 초음파 발진회로 채널 3 ON/OFF |
| PB14 | CTRL_OSC4 | **mega16 PC7** | 초음파 발진회로 채널 4 ON/OFF |
...
| I2C1 | PB6/PB7 | EEPROM |
| ADC1 | PB0/PB1 | 센서 아날로그 입력 2채널 |
| TIM9_CH1 | PA2 | 부저 PWM |
| TIM5_CH1 | PA0 | 초음파 PWM 출력 |
| TIM11 | — | 내부 타이머 (용도 TBD) |
| RTC | — | 실시간 클럭 (LSI 32 kHz) |
| SWD | PA13/PA14 | 디버그/플래시 |
| GPIO Out | PB2–PB5, PB10, PB12–PB14, PC5 | 초음파 발진회로/커넥터 제어 |
| GPIO In | PA11/PA12/PA15, PB11, PC4, PC10–PC12 | 센서/키/이벤트 입력 |
...
| CTRL_OSC0–4 active level | 외부 제어 출력 확정. **활성 극성은 추후 확정 (Q8 유보 — Stage D `do_control()` 구현 시)** |
| ADC1 IN9 (PB1) | 채널 활성 필요 (Stage D mega16 ADC 평균화 흡수 시점) |
...
| `M_OVLD`  | ATmega16 → SAMD20 | PB10 (in)  | PC0 (out) | LOW(추정) | 과부하 알람 |
...
| PB13 | `CTRL_OSC3` (out) | 이전 M_START 자리 |
