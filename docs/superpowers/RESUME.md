...
git checkout feat/stage-c-modbus-tcp-dhcp       # slice 2b: 코드+HW E2E 완료, tip=62c1cc1 (base = 2a tip faf89d5, stack, 미머지)
env -u STM32_TOOLCHAIN cmake -S fw -B fw/build -G Ninja && env -u STM32_TOOLCHAIN cmake --build fw/build  # 0-warning(우리 코드), 2b text 52728B (FLASH 40.65%)
make -C fw/test test                            # 호스트 3스위트 PASSED (2b는 새 호스트 테스트 추가 ✗ — DHCP=HAL/소켓/콜백 글루)
# 진행 상태: slice 2b 코드(T1~T4)+advisor MAC fix+비블로킹 PHY 리팩터(635e9ec)+리뷰 fix(62c1cc1) 완료. HW E2E PASS(static+DHCP+재시도 복구). 자세히=changelog 2026-06-13 h.
# === 다음 = 머지 전략 결정 (리팩터로 2b app_eth가 2a와 byte-identical 아님) ===
#   문제: slice 2a 원본 app_eth는 블로킹 1s PHY 폴 = confirmed HW 버그 + HW 미검증. static 경로는 2b(리팩터) 빌드에서만 HW 검증됨.
#   ★권장(기본) 옵션 B: 2a/2b split 합쳐 2b 통째 머지(2b가 2a 전부 포함+검증, 2a static은 strict subset). ⚠ pre-refactor 2a에 c2a 태그 금지(known-broken).
#    옵션 A: 2a가 비블로킹 리팩터 cherry-pick → 2a HW 재검증 → 2a 머지 → 2b 머지. 가능하나 busywork(재검증해도 얻는 것 없음).
#   결정 후 finishing-a-development-branch 머지(--no-ff) + 태그 (Option B면 hw-revA_fw-stage-c2b 단일, 또는 사용자 결정)
#   선택 deferred HW(사용자 "핵심 충분"): ICON_RUN 육안(START→560ms ceiling→STOP over TCP), RTU FC06 회귀(RS-485), RAM-only 재리스(static 복귀→.128)
# HW(보드): ST-LINK V3. 플래시 = openocd -f fw/openocd/stm32f410.cfg -c "program fw/build/gds_us_ctrl.elf verify reset exit"
# mon@115200(리다이렉트형식 필수, 인자형식 cat은 9600 reset됨): { stty 115200 cs8 -parenb -cstopb raw -echo; exec cat; } < /dev/cu.usbserial-AB0MLYXA > /tmp/eth-mon.log & ; 읽기 LC_ALL=C tr -d '\000'
# ⚠ slice 2a HW E2E(Task9 static)는 2b에서 ETH_STATIC로 검증됨(동일 빌드); 2a 원본 코드는 PHY 버그라 별도 검증 불가 → 머지 전략(위)으로 흡수
# (옛 Stage D + m1 = MERGED; modbus slice 1 = MERGED; slice 2a 코드 = 완료 2026-06-13 e; slice 2b 코드+HW = 완료 2026-06-13 h)
```
