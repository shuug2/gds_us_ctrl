...
#   문제: slice 2a 원본 app_eth는 블로킹 1s PHY 폴 버그 보유 + HW 미검증. static 경로는 2b(리팩터) 빌드에서만 HW 검증됨.
#   옵션 A: 2a 브랜치가 비블로킹 리팩터를 cherry-pick/상속 → 2a HW 재검증 → 2a 머지 → 2b 머지 (2a→2b 순서 유지)
#   옵션 B: 2a/2b split 합쳐 2b를 통째로 머지(2b가 2a 전부 포함) → 태그 한 번 또는 c2a+c2b 연속
#   결정 후 finishing-a-development-branch 머지(--no-ff) + 태그 (hw-revA_fw-stage-c2a / c2b)
