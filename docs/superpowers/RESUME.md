...
git checkout feat/stage-c-modbus-tcp-dhcp       # slice 2b: 코드 완료(host-complete), tip=b7ce6b0 (base = 2a tip faf89d5, stack)
env -u STM32_TOOLCHAIN cmake -S fw -B fw/build -G Ninja && env -u STM32_TOOLCHAIN cmake --build fw/build  # 0-warning(우리 코드), 2b text 52680B (FLASH 40.62%)
make -C fw/test test                            # 호스트 3스위트 PASSED (2b는 새 호스트 테스트 추가 ✗ — DHCP=HAL/소켓/콜백 글루)
# 진행 상태: slice 2b plan(166fe78, 5 task) Task 1~4 완료(코드 6커밋 3a62233/5b84f71/a403326/08b4004/86a3923 + advisor fix b7ce6b0, 통합 cpp-reviewer APPROVED-WITH-COMMENTS Crit/High 0). Task 5(HW E2E)만 보드 게이트.
