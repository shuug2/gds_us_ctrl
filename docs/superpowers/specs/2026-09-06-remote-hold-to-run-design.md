...
>
> **사용자 결정(2026-09-06, §0)**: 착수 = **PC8 과 독립(순서 결정 불요)** · STD 빌드 **포함**(`#if` 0개) · 패널 TOUCH
...
| # | 결정 | 귀결 |
|---|---|---|
| D | **PC8 과 독립 — 순서 결정 자체가 불요** | 처음엔 "PC8 앞/뒤" 결정으로 올렸으나 사용자 재질문으로 정정. hold 워치독은 PC8 을 건드리지 않고(`app_reg` 열거·게이트 분기·극성 매크로 전부 무변경), host + TCP 벤치로 닫히며, 단절 시 30 s→0.6 s 라 안전 방향이다. "인터록 부재를 가린다" 우려는 PC8 이 운전 중 차단층일 때만 성립하는데 PC8 은 설정 스위치다(아래). 배포 금지 해제 조건도 이 작업과 무관. 남는 건 **벤치 세션을 PC8 재시험과 섞지 않는다**는 운영 규칙뿐 |
...
2. **D 가 결정이 아닌 이유** — "hold-to-run 이 인터록 부재를 가린다" 는 우려는 PC8 이 운전 중 차단층일 때 성립한다.
   설정 스위치면 가릴 층이 없다. 정상 경로만 보면 hold(단절 시 0.6 s)는 현행 탭(30 s)보다 **안전 방향**이다.
   → 순서를 정할 이유가 없다. "앞/뒤" 프레임 자체가 틀렸고 정확한 프레임은 **"독립"** 이다.
...
| H-16 | — | 회귀: `gds_us_hmi`/mbpoll 흐름 START=1·STOP=1·cfg 쓰기·CFG_CAP `0xFA01` · **START=99 → STOP=1**(비-1 값 소거가 STOP 을 한 프레임 지연시키지 않는지) | 무회귀 |
...
**H. 다중 마스터 각주 (코드 변경 불요)** — hold 런 도중 링크가 끊겼다 T 안에 **다른** 클라이언트가 붙어 START=3 을 보내면 그 세션을 이어받는다. keep 은 기동 권한이 없으므로 새 런을 만들지는 못하고, 이는 "아무 마스터나 START=1 을 쓸 수 있다" 는 기존 Modbus 표면과 동급이다. 기록만.
