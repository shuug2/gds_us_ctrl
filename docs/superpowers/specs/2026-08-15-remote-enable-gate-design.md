...
>
> ⚠ **HW 재시험 필요**: VR-7(아래) 은 "해제 후에도 재활성 전 START 불가"를 요구하지만
> 이제 **해제 즉시 재활성**이 정답이다. VR-7 을 그 판정으로 바꿔 재시험할 것.
...
| VR-7 | EMSW(PC11, model_type=std) E-STOP → 즉시 `=4`, 해제 후에도 재활성 전 START 불가. ⚠ rig 노트 R1 |
| VR-8 | LCD 수동 해제 → `=5` |
