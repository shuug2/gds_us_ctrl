...
## 4c. 신규 발견 — M16 disasm 확보 후 (OVLD · PA7 · PA0 vestigial)

**2026-06-20 후속**: 사용자가 `ref/atmega16/`에 M16 펌웨어 원본 추가 — `m16_controller_prg.hex` + `m16_disasm/firmware_disassembled.asm`(2953줄 전체 디스asm) + `avr_disassembler.py` + `firmware_analysis.md`. **이전 "raw .asm 없음" 상태 해소 → M16 로직 직접 추적 가능.** ⚠ 일부 명령 미디코드 `.word`(예 `0x107C`) 잔존.

### OVLD(PC0) 구동 로직 (firmware_disassembled.asm)

검출 함수 `@0x10A6` (per-tick 호출 `0x06EC`):

```
0x10C0 sbis 0x19,7    ; PA7(PINA.7) 읽기
0x10C2 rjmp 0x10EE    ;   PA7==0 → 카운터(0x0199)=0, assert 안 함 (noise reject)
0x10C4..              ;   PA7==1 → 카운터++
0x10D2 cpi r26,0x05   ;   카운터 ≥ 5 →
0x10E6 ..0x019C=1     ;   issued 래치 + 0x01A2=1
0x10EA sbi 0x15,0     ; ★ PC0 = HIGH = M_OVLD 어서트
```

- **로직 = PA7 HIGH 연속 5샘플 디바운스 → PC0(M_OVLD) HIGH.** 재진입 게이팅 = `0x019C`(issued)/`0x01C5`.
- **복구 훅**: overload 경로(`0x08CC → call 0x1D28`)가 **SEEK 플래그 `0x018A`=1** set → **OVLD→SEEK 자동 복구**(IPC 가설 확증).
- **해제**: reset/clear-all(`0x1B7A`/`0x1CFE`/`0x1FC0`, PC1/PC6와 함께 `cbi 0x15,0`).
- **극성**: PA7 풀업 idle HIGH인데 HIGH=fault-카운팅 → 정상시 외부가 LOW로 잡고 fault/단선시 HIGH(fail-safe) 추정 → **보드 실측 확정**.

### ★ PA7 = 과부하 입력 → 보류했던 OSC3 해소
...
- **SAMD20**: PA0 네트 → PB03(AIN11=`ADC_OUT_LV`) → `curr_lv` 획득·래치(`last_lv`)하나 **유일 소비처 `send_outpower_data(curr_lv/40,…)`는 주석처리(main.c:5135)** → 출력파워 표시는 `curr_amp`(소비전류)로 전환 = **curr_lv 실사용 없음**.
- **함의**: STM32 `reg_scale(ch0=PA0)→curr_power→DISP_POWER`는 **레거시 양쪽이 안 쓴 신호를 "전력"으로 오용**(전력=PB1 소비전류×2.2, §4b). **ch0(PA0/출력세기) 통합 역할 = 미결정**(curr_lv 표시 부활 / vestigial 유지 / 기타).

### v001 신뢰도 (단독 금지 재확인)

v001 누락/오류 누적: ×6(`>>6` 오기)·lookup `<`·warmup·**ADMUX REFS=11(AVCC 오기)**·OSC 변수명 오라벨·**PA5/PA6 read 0건**·**PC4 read 누락**·**overload 로직 전체 누락**. → **권위 = `firmware_disassembled.asm`, v001 = 참고용만.**
...
5. PB13=과부하 입력 확정(§4c, PA7) → OVLD 극성 실측(정상 LOW? fault HIGH?). PB12(OSC2/PC4)만 방향 미확정 → 외부 구동 여부 관측
...
- **`ref/atmega16/m16_disasm/firmware_disassembled.asm`** (2953줄, 권위) — OVLD fn `@0x10A6`(PA7 디바운스→`sbi 0x15,0`@0x10EA), 호출 `0x06EC`, 복구 `0x1D28`(0x018A=1), clear `0x1B7A`/`0x1CFE`/`0x1FC0`. + `m16_controller_prg.hex`, `firmware_analysis.md` (사용자 2026-06-20 추가)
- `ref/atmega16/m16_conv_v001.c` — 참고용(부분·부정확): init 1151-1168, OSC 미러 453-473, scaling 672-695, PINA.4 580. ⚠ overload/PA5/PA6/PC4/ADMUX-REFS 누락·오기 → disasm 우선
- `ref/samd20/main.c` — curr_amp/power/energy(412-436), ADC_OUT_LV→curr_lv(439), curr_lv 소비처 주석(5135), 채널 PIN0/PIN11(344-353)
