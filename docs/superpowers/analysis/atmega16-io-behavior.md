...
>
> ⚠️ **2026-05-25 (후속) 보드-진실 정정 — 본문보다 §0.1 우선.** 사용자 보드 직독으로 OSC 드라이브 핀 매핑이 확정되며 본문의 일부 결론(§0/§1/§3/§4/§7)이 뒤집힘. **소스 오브 트루스 전환**(디컴파일 → 보드 측정). 아래 §0.1을 먼저 읽을 것.
...
## 0.1 ⚠️ 보드-진실 정정 (2026-05-25 후속, 본문보다 우선)

> 사용자 보드 직독으로 OSC 드라이브 매핑이 확정되며 본 문서의 디컴파일 기반 결론 일부가 **뒤집힘**.
> **소스 오브 트루스 전환**: 이제 1차 = 사용자 보드 측정 + 행동 재분석, 2차(약함) = `m16_conv_v001.c` 디컴파일 (**함수명 무시**, 동작은 재도출). 아래 표가 본문 해당 절보다 우선한다.

### 바뀐 부분 (보드-진실로 뒤집힘)

| # | 본문 기재 | 정정 |
|---|---|---|
| 1 | OSC 트리거 물리 출력핀 = **미해소** (§7 #1) | **확정**: `CTRL_OSC0..4` ← mega16 **PB1/PB0/PC4/PA7/PC7** → V30 PB2/PB10/PB12/PB13/PB14 (`docs/pinmap.md`) |
| 2 | IPC = PB0/PB1/PA7/PC4/PC7 순수 GPIO (§1) | **틀림**. 그 5핀 = **OSC 드라이브 채널**. 실제 IPC = 명령선 M_START/SEEK/RESET(PA4/5/6) + M_OVLD(PC0) 별개 그룹 |
| 3 | PB0=`g_run_flag` / PB1=`g_blink_active` / PC7=`g_blink_state` IPC 상태표시 (§3) | **무시** (디컴파일 함수명 추정). 실제 = OSC 채널 거동 |
| 4 | 누진 패턴 → dead display 로만 감 (§0/§4) | **수정**: 드라이브는 실제 OSC핀에 도달 (가시 코드 밖 — `output_direct_process` 본문 부재 추정) |
| 5 | V26 net 라벨 `U1.40/41/26/30/23` = SAMD20 행 (§3) | 보드-진실은 OSC 드라이브 — 육안 net 라벨 신뢰 약화 |

### 확정해야 할 부분 (사용자 재측정 + 컨펌 대기 — Stage D 코딩 차단)

| # | 항목 | 비고 |
|---|---|---|
| B1 | **5개 OSC핀 각 방향** (출력 vs 입력) | PC4·PA7는 코드상 입력(DDRC bit4=0, DDRA=0x00) — "순수 5출력"이 아닐 수 있음(OSC보드→M16 피드백 입력 섞임?) = 루프 토폴로지 결정 |
| B2 | **레벨→OSC 비트매핑 + 극성** | 5비트 누진이 5핀 비트순 1:1? Active H/L? |
| B3 | **레귤레이션 전달함수** (PA0 피드백 → scaled → 레벨) | 디컴파일 스케일 수식 placeholder. 동작 재도출 |
| B4 | **ADC ch1(PA1, 50샘플) 정체** | PA0=출력세기 **확정**. PA1은? (코드는 무겁게 처리, 회로도 미연결) |
| B5 | **`g_main_state` 0/1 두 출력모드 의미** | lookup 레귤레이션(state0) vs `output_direct_process`(state≠0) |

확정 유지: **PA0(ADC0) = 출력 초음파 세기 피드백** (레귤레이션 입력). CTRL_OSC 핀 **identity** = PB1/PB0/PC4/PA7/PC7.

> **결과**: Stage D는 "ref verbatim 포팅" 불가. 구조부(ADC 획득층·TIM cadence·상태머신 골격·출력드라이버 추상화)만 선포팅 가능하며, 전달함수·핀방향·매핑은 B1~B5 측정 후 확정. Stage D 코딩은 측정 대기로 보류.
...
| # | 질문 | 왜 중요 | 검증 방법 |
|---|---|---|---|
| ~~**1**~~ | ~~**OSC 트리거 물리 출력 핀**~~ → **§0.1로 해소**: PB1/PB0/PC4/PA7/PC7 (보드 직독). 단 각 핀 방향(B1)·극성/비트매핑(B2)은 측정 대기 | 흡수의 출력단 매핑 | **CLOSED** (identity). 방향·극성 = §0.1 B1/B2 |
