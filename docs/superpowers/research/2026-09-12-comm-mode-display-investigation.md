...
> **종결 (2026-09-13)**: 사용자가 **DGUS 프로그램에서 패널 자산을 수정해 해결** — 원인은 펌웨어가 아니라 패널 자산(페이지 27 의 `0x140c` 아이콘 컨트롤)이었다. 아래 요약의 "미확정(추측)" 이 실기로 확정된 것. 펌웨어 측 시도 B(set_page 뒤 100 ms 지연 재기록, `9fc94df`)는 효과 없음이 실증돼 **원복**(`3be6a5d`). 사용자 회신(2026-09-13): **"버전이 잘못되었었던듯"** — 패널에 실려 있던 자산이 저장소 `hw/lcd/dgus/`(05-31 수정본)와 다른 구버전이었던 것으로 판단(06-08 재플래시 추측 부합). 수정 자산의 저장소 반영 여부는 **다음 세션 확인 항목**(패널본 = 저장소본이면 조치 없음). 2026-05-31 분석의 "패널 특성" 서술은 "패널에 실린 자산의 문제" 로 읽어야 한다.
