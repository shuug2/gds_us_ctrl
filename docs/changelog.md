...
- **fix(lcd) B — COMM 모드 아이콘 지연 재기록**(같은 벤치 빌드): 전원 재투입 후 COMM 페이지에 처음 들어가면 **화면은 이더넷 페이지(STD = page 27)로 맞게 진입하는데 모드 선택 아이콘만 "시리얼"** 로 보이던 결함. page 27 은 표시 시 `DISP_COMM_MODE` 를 **auto-load 하지 않고 활성 중 라이브 쓰기만 반영**(2026-05-31 `analysis/2026-05-31-std-comm-page27-display-port-faithful.md`)하는데, 그때 넣은 set_page 직후 재기록(Fix C)이 이 보드에서는 **패널이 페이지를 그리기 전이라 유실**된다. `app_lcd_tick` 에서 set_page 후 `LCD_COMM_ICON_REASSERT_MS`(100 ms) 경과 시 **1-shot 재기록**(state 필드 `comm_icon_reassert_pending` 무장 + 발화 시 현재 페이지가 여전히 comm 인지 확인). `render_comm_tail` → 공개 `app_lcd_comm_icon_refresh(page)` 승격(Fix C 와 값·소스 동일, 삭제 아님 — MULTI page 25 는 auto-load 로 이미 정상이라 중복 무해). **실기 3단계 판별**(TCP 접속 OK · 이더넷 페이지 진입 · 아이콘만 시리얼)과 **원격기 회신**(부팅·접속 직후 FC06 0건, `0x21` 쓰기 경로 없음)으로 cfg·FRAM·부팅 로드·TCP 개통·원격기 쓰기를 모두 배제해 **표시 한 지점**으로 확정. 에코 브랜치와 무관한 **기존 결함**이며 legacy samd20(`main.c:3159-3172`)은 set_page 전에만 써서 같거나 더 나쁨. 조사 `research/2026-09-12-comm-mode-display-investigation.md`, spec `specs/2026-09-13-comm-icon-deferred-reassert-design.md`. 벤치 E-17(재투입 후 아이콘 육안, 실패 시 200 ms 로 1회 재시험) + E-17b(진입 직후 이탈 시 잔상 없음).
...
- **app_modbus.c**: 이력 서술(2026-09-04/05 fix 경위 등, changelog 에 있는 것만) 압축 + 리플로로 815→753(커밋 시점 752 + K12 안전 근거 복원 1), 헬퍼 6개 추가 후 792(+39 = 헬퍼 시그니처·중괄호·1줄 주석·빈 줄).
...
- 최종 리뷰 fix: `bin-same.sh` errexit 분리, 주석 표류 5곳, dispatch 189.
- 기준 해시는 시작 커밋 `b61ef0f` 클린 빌드에서 `fw/tools/bin-same.sh baseline` 으로 재생성 가능(툴체인 arm-none-eabi-gcc 15.2.1 고정).
...
## [3.1.0] — 2026-09-05 · tag `hw-revA_fw-3.1.0`

> STD `V3.0.0_260905` / REMOTE `V3.1.0R!_260905`. **번호 = 기능 티어, 날짜 = 그 빌드** —
> STD 는 레거시-동등 기준선이라 번호를 동결한다(사용자 결정 2026-09-05). 3.0.0 이후 62커밋.
> 통합 벤치 = 실행 41 + 회귀 27 항목 전건 PASS (`plans/2026-09-05-bench-results.md`).
>
> 🔴 **배포 금지** — PC8 인터록 극성이 반전돼 있다(`REMOTE_EN_INTERLOCK_INVERTED 1`).
> 단선·커넥터 탈락이 "허용" 이 된다. 해제 = 실장 PCB + 매크로 `0` + A-1·A-5·A-13 재시험.
> 버전 문자열의 `!` 가 그 상태의 표식이고, 매크로를 따라가므로 어긋날 수 없다.

### 2026-09-05 — release: 3.1.0 컷 — 티어 버전 체계 + 인터록 극성 매크로화

- **버전 체계**: `3.0.x` = STD(레거시-동등) / `3.1.x` = REMOTE. 접미어 `R` = 모델, `!` = 극성 반전판. 세 문자열 전부 20자(`app_lcd_render.c` `_Static_assert`).
- **STD 번호 동결**: `V3.0.0` 유지, 날짜만 `_260725`→`_260905`. Modbus 소비자의 capability 판별은 버전이 아니라 `0x31 CFG_CAP`/`0x2A REMOTE_CAP` 이 하므로(버전 레지스터 없음) 동결이 소비자를 오도하지 않는다. ⚠ STD 에도 IWDG · Modbus 무음 손상 fix 2건 · F-A 레지스터 확장이 들어가 있다 — 날짜가 그 구분자다.
- **극성 매크로화**: `define.h` `REMOTE_EN_INTERLOCK_INVERTED` 신설. `app_modbus.c:remote_en_step()` 의 `in.sw` 판정과 버전 표식 `!` 가 **같은 값**을 따라간다 → 원복(`1`→`0`) 한 글자로 둘이 동시에 움직인다.
- **거동 변화 없음**: 반전(=1) 판의 `in.sw` 식은 종전과 동일. 두 모델 바이너리가 **버전 문자열 바이트 외 완전 동일**함을 `cmp` 로 확인(STD 2B / REMOTE 9B, 전부 문자열 영역 안).
- 게이트: STD·REMOTE 2종 0-warning(FLASH 50.67% / 50.90%) · host 16스위트 PASS · `=0` 원복 분기도 빌드 확인. ⬜ LCD 버전 육안(`!` 렌더 포함) = 다음 보드 세션 첫 항목.

### 2026-09-05 — feat(modbus): STATUS 에 SEEK/RESET 진행 비트 (`405f95e`, 원격기 요청)

- `bit7 = MB_STATUS_SEEK(0x80)` / `bit8 = MB_STATUS_RESET(0x100)`. RESET→SEEK 자동 체인이라 **순차로** 뜬다(FSM 단일 상태 = 동시 점등 구조적 불가). 600ms/leg.
- **근거**: 소비자가 체인 길이를 숫자로 베껴 방어하다 hold(1000/500)가 실측 무음 창(1200/600)보다 짧아 **버튼을 먼저 열고 있었다**. 명령 4종 중 RESET·SEEK 만 "전송됨·확인 불가" 였다.
- 구현 ~10줄 — `mb_status_bits()` + 미러(`seek_reset_fsm_state()` 직접 read) + host 테스트. `app_reg`·FSM·글루 무변경.
- 🔴 **capability 판별자 없음(사용자 결정)** — 구 펌웨어에서 이 비트는 영영 0 이라 소비자가 미지원과 구분할 수 없다. **비트 0 을 "미진행" 근거로 쓰는 방어 로직 금지, 비트=1 만 정보다.**
- 벤치 PASS: RESET 3회 `0x0100` · SEEK 1회 `0x0080`, 겹침 0건. ⚠ 순차 순서의 증거는 육안뿐(밀리초 정본 = 컨트롤러 mon).

### 2026-09-05 — feat(remote-en): E-STOP 래치 폐기 — 레벨 추종으로 전환 (`23ee66c`)

- `REMOTE_EN` 이 `DIS_ESTOP(4)` 를 래치하지 않는다. E-STOP 이 풀리면 게이트가 스스로 `1` 로 복귀한다.
- **원인**: 래치의 유일한 청소 경로가 스위치 재무장(`in->sw == 0u`)인데 PC8 미실장 + 극성 반전으로 그 경로가 **물리적으로 닫혀 있었다** → E-STOP 한 번이면 재부팅 전까지 원격 제어 사망(원격기 실측 8분 무전이).
- **남는 E-STOP 차단 3층**: `app_reg.c` START 가드(레벨·소스 무관) · `app_input.c` 매 tick `RUN_RELEASE`+SOL OFF · 원격기 `gds_safety` STATUS bit1. 잃은 것은 "해제 후 재확인 의식" 뿐.
- VR-7 재시험 PASS — `4→1` 자가복귀 6회(구 펌웨어 대조군 0건). ⚠ **짧은 인가는 `4` 를 통째로 놓친다 — 상태 부재를 미동작 근거로 쓰지 말 것.**

### 2026-09-05 — feat(remote-en): PC8 극성 반전 — 인터록 미실장 구간 한시 조치 (`8f33c5f`)

- 원안 active-LOW fail-safe(닫힘=LOW=허용 / 열림·단선·탈락=HIGH=불허)를 뒤집어 미실장(풀업 HIGH)을 "허용" 으로 읽는다. 추가 회로 없이 원격 제어를 열기 위함(사용자 결정).
- 🔴 **이 상태에서는 인터록 보호가 없다** — 단선·커넥터 탈락이 허용이 된다. 배포 금지의 사유.
- 3.1.0 에서 `define.h` 매크로로 승격(위 release 항목) — 원복 지점이 한 곳이고 버전 표식이 따라온다.
...
- **원인**: `mirror_live()` 는 `holding[WORK_CNTL] = (uint16_t)work_cnt` 하위-워드 미러라 `k·65536` 에서 **미러 자신이 0 을 만든다.** `0ab2608` 이 `(uint16_t)` 캐스트를 제거하면서, 그 캐스트가 유일하게 해 주던 "미러가 만든 0 vs 마스터가 쓴 0" 구분이 사라졌다. 커밋 메시지의 거동표가 "마스터가 0 을 쓴 경우" 열만 다루고 미러 열을 빠뜨린 것이 놓친 지점.
- **수정**: 판정을 순수 술어 `mb_work_cnt_reset_req()`(`app_modbus_core.c`)로 추출하고 `last_write_addr == MB_REG_WORK_CNTL` 을 AND 조건에 추가. 같은 글루의 staged 스캔이 쓰는 기법·선례와 동일.
- **거동 변화**: `k·65536` 에서의 가짜 리셋만 사라진다. 정상 리셋(CNTL=0 쓰기)은 모든 값에서 그대로 — **`0ab2608` 의 승인된 samd20 이탈은 유지**되고 미승인 부작용만 제거. 계약·미러·다른 분기 무변경.
- **레거시**: samd20 은 하위 워드만 비교해 `k·65536` 에서 리셋을 무시했다(무해·자기치유). 즉 **현행이 레거시보다 위험했다** — 필드 배포 전 필수 항목이었다.
- **테스트**: host 5케이스 `test_work_cnt_reset_req` 신설 — 65536 회귀 고정 포함. **벤치로는 재현 불가**(65536 사이클 필요 / Modbus 설정 불가 / SWD 쓰기 금지) → host 가 유일한 게이트. 가드 제거 시 실패함을 확인.
- 게이트: our-code 0-warning, host 16스위트 PASS, FLASH 50.55 %.
- ⚠ **별건 미처리**: LCD/weld 가 같은 iteration 에 cfg 를 바꾼 뒤 FC06 이 오면 되돌리는 **1-iteration stale-미러 레이스**(모든 compare 분기 공통, samd20 에도 있던 것). 전면 해결은 체인을 주소-디스패치로 바꾸는 설계 변경이라 번들하지 않았다.
...
## [3.0.0] — 2026-08-30 · tag `hw-revA_fw-3.0.0`

> `V3.0.0_260725` — STM32F410 통합 포트 완료, 기능 티어(STD/REMOTE) 분기 이전의 **레거시 출하 기준점**.
